...
const NotesList = () => {
  ...
  const tableContent = ids.map((id) => {
    ...
    const rowStatus = note.completed ? "table__row--completed" : "table__row--pending";
    ...
    return (
      <tr key={id} className={`table__row note ${rowStatus}`}>
        ...
        <td className="table__cell note__created">{formatDate(note.createdAt)}</td>
        <td className="table__cell note__updated">{formatDate(note.updatedAt)}</td>
